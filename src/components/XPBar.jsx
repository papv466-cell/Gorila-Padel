  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
      <span style={{
        ...
        border: "1px solid rgba(var(--sport-color-rgb, 46,204,113),0.3)",
...
export default function XPBar({ userId }) {
  ...
  return (
    <div style={{ background: "#111", borderRadius: 16, border: "1px solid rgba(255,255,255,0.08)", overflow: "hidden", marginBottom: 16 }}>
      ...
      {/* HEADER — nivel y XP */}
      <div style={{ padding: "16px 16px 12px", background: "rgba(var(--sport-color-rgb, 46,204,113),0.06)", borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 10 }}>
          <div>
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              <span style={{ fontSize: 22, fontWeight: 900, color: "var(--sport-color)" }}>Nivel {levelInfo.level}</span>
        ...
        {levelInfo.next && (
          <div style={{ background: "rgba(255,255,255,0.08)", borderRadius: 999, height: 8, overflow: "hidden" }}>
            <div style={{
              ...
              background: "linear-gradient(90deg,var(--sport-color),var(--sport-color-dark))",
      ...
      <div style={{ display: "flex", borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
        {[
          ...
          <button key={t.id} onClick={() => setTab(t.id)} style={{
            ...
            background: tab === t.id ? "rgba(var(--sport-color-rgb, 46,204,113),0.1)" : "transparent",
      ...
      {tab === "stats" && (
        <div style={{ padding: 16 }}>
          ...
          {/* Próximo nivel */}
          {levelInfo.next && (
            <div style={{ marginTop: 12, padding: 12, borderRadius: 12, background: "rgba(var(--sport-color-rgb, 46,204,113),0.06)", border: "1px solid rgba(var(--sport-color-rgb, 46,204,113),0.15)" }}>
              <div style={{ fontSize: 12, color: "rgba(255,255,255,0.5)", marginBottom: 4 }}>
                Próximo nivel: <span style={{ color: "var(--sport-color)", fontWeight: 900 }}>{levelInfo.next.label}</span>
              </div>
          ...
          {ACHIEVEMENTS.map(a => {
            ...
            return (
              <div key={a.key} style={{
                ...
                background: unlocked ? "rgba(var(--sport-color-rgb, 46,204,113),0.08)" : "rgba(255,255,255,0.03)",
                border: unlocked ? "1px solid rgba(var(--sport-color-rgb, 46,204,113),0.25)" : "1px solid rgba(255,255,255,0.05)",
                ...
                {a.xp > 0 && (
                  <div style={{ fontSize: 10, color: "var(--sport-color)", fontWeight: 800, marginTop: 4 }}>+{a.xp} XP</div>
                )}
                {unlocked && unlockedData && (
                  <div style={{ fontSize: 9, color: "rgba(var(--sport-color-rgb, 46,204,113),0.6)", marginTop: 3 }}>
        ...
        <div style={{ padding: 12 }}>
          <div style={{ display: "flex", gap: 8, marginBottom: 12 }}>
            {[
              ...
              <div key={s.label} style={{ flex:1, background:"rgba(var(--sport-color-rgb, 46,204,113),0.06)", border:"1px solid rgba(var(--sport-color-rgb, 46,204,113),0.12)", borderRadius:10, padding:"8px 6px", textAlign:"center" }}>
                <div style={{ fontSize:16, fontWeight:900, color:"var(--sport-color)" }}>+{s.value}</div>
            ...
            recentXp.map((e, i) => {
              ...
              return (
                <div key={i} style={{ display: "flex", justifyContent: "space-between", alignItems: "center",
                  ...
                  background: isAchievement ? "rgba(var(--sport-color-rgb, 46,204,113),0.06)" : "rgba(255,255,255,0.02)",
                  border: isAchievement ? "1px solid rgba(var(--sport-color-rgb, 46,204,113),0.15)" : "1px solid rgba(255,255,255,0.04)" }}>
                  ...
                  <span style={{ fontSize: 15, fontWeight: 900, color: "var(--sport-color)", whiteSpace:"nowrap" }}>+{e.xp} XP</span>
                </div>
              );
            })
          )}
