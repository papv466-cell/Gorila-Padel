...
function normText(s) { return String(s||"").trim().toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g,""); }
...
function setActiveClass(id, endAt) { try { localStorage.setItem(LS_ACTIVE_CLASS_ID, String(id||"")); localStorage.setItem(LS_ACTIVE_CLASS_END_AT, String(endAt||"")); } catch {} }
function clearActiveClass() { try { localStorage.removeItem(LS_ACTIVE_CLASS_ID); localStorage.removeItem(LS_ACTIVE_CLASS_END_AT); } catch {} }
function getActiveClass() { try { return { id: String(localStorage.getItem(LS_ACTIVE_CLASS_ID)||"").trim(), end_at: String(localStorage.getItem(LS_ACTIVE_CLASS_END_AT)||"").trim() }; } catch { return { id:"", end_at:"" }; } }
...
function getNextDays(n=14) {
  ...
  for (let i=0; i<n; i++) {
    ...
    days.push({ value: toDateInputValue(d), label: d.toLocaleDateString("es-ES",{weekday:"short"}), num: d.getDate(), isToday: i===0 });
  }
  return days;
}

export default function ClassesPage({ session: sessionProp }) {
  ...
  const [myBookings, setMyBookings] = useState(new Set()); // IDs de clases que yo reservé
  ...
  const [slotMaxStudents, setSlotMaxStudents] = useState(1);
  ...
  // Valoración post-clase
  const [ratingModal, setRatingModal] = useState(null); // { classId, teacherId, teacherName }
  const [ratingValue, setRatingValue] = useState(0);
  const [ratingComment, setRatingComment] = useState("");
  const [ratingSending, setRatingSending] = useState(false);

  useEffect(() => { fetchClubsFromSupabase().then(r=>setClubsSheet(Array.isArray(r)?r:[])).catch(()=>setClubsSheet([])); }, []);

  const clubsAll = useMemo(() => (Array.isArray(clubsSheet)?clubsSheet:[]).map(r=>({
  ...
  })).filter(x=>x.id&&x.name), [clubsSheet]);
  ...
  useEffect(() => {
    if(!authReady)return;
    const t=searchParams.get("teacher"); if(t&&isUuid(t))setFilterTeacher(String(t));
    const clubId=String(searchParams.get("club")||"").trim();
    if(clubId){ setFilterClubId(clubId); const cn=String(searchParams.get("clubName")||"").trim(); if(cn)setFilterClubQuery(cn); }
  }, [authReady]);
  ...
  const slotClubSuggest = useMemo(() => { const q=normText(slotClubQuery); if(!q||q.length<2)return []; return clubsAll.filter(c=>normText(c.name).includes(q)||normText(c.city).includes(q)).slice(0,14); }, [clubsAll,slotClubQuery]);
  useEffect(() => { const q=normText(slotClubQuery); if(!q){setSlotClubPick(null);return;} setSlotClubPick(clubsAll.find(c=>normText(c.name)===q)||null); }, [slotClubQuery,clubsAll]);
  const filterClubSuggest = useMemo(() => { const q=normText(filterClubQuery); if(!q||q.length<2)return []; return clubsAll.filter(c=>normText(c.name).includes(q)||normText(c.city).includes(q)).slice(0,14); }, [clubsAll,filterClubQuery]);
  useEffect(() => { const q=normText(filterClubQuery); if(!q){setFilterClubId("");return;} const exact=clubsAll.find(c=>normText(c.name)===q); if(exact?.id)setFilterClubId(exact.id); }, [filterClubQuery,clubsAll]);
  ...
  async function loadDay() {
    try {
      ...
      if(uid) {
        ...
        if(ids.length) {
          const {data:bk} = await supabase.from("class_bookings").select("class_id").eq("user_id",uid).in("class_id",ids);
          setMyBookings(new Set((bk||[]).map(b=>String(b.class_id))));
        } else setMyBookings(new Set());
      }
      ...
      const {data:profs} = await supabase.from("profiles").select("id,name,handle,avatar_url,xp").in("id",teacherIds);
  ...
  async function createClass() {
    ...
    try {
      ...
      const {error} = await supabase.from("classes").insert({
        ...
        max_students:Number(slotMaxStudents)||1,
        location:slotLocation?.trim()||null,
        club:slotClubPick.name, club_id:slotClubPick.id, club_name:slotClubPick.name,
        club_city:slotClubPick.city||null, club_lat:Number.isFinite(slotClubPick.lat)?slotClubPick.lat:null,
      ...
      setSlotPrice(""); setSlotLocation(""); setSlotNotes(""); setSlotCourt(""); setSlotMaxStudents(1);
  ...
  async function bookClass(classId) {
    ...
    try {
      ...
      setMyBookings(prev=>new Set([...prev,String(classId)]));
  ...
  async function cancelMyBooking(classId) {
    ...
    try {
      ...
      setMyBookings(prev=>{ const s=new Set(prev); s.delete(String(classId)); return s; });
  ...
  async function cancelClass(classId) {
    ...
    if(!cls)return;
    const msg=cls.is_booked?"Esta clase está reservada.\n\nSe cancelará y se avisará al alumno.\n¿Continuar?":"Esta clase está libre.\n\nSe borrará definitivamente.\n¿Continuar?";
  ...
  async function sendRating() {
    ...
    try {
      await supabase.from("teacher_ratings").upsert({
        user_id: uid,
        teacher_id: ratingModal.teacherId,
        class_id: ratingModal.classId,
        rating: ratingValue,
        comment: ratingComment.trim()||null,
      }, { onConflict: "user_id,class_id" });
      toast.success("¡Valoración enviada! 🦍");
      setRatingModal(null); setRatingValue(0); setRatingComment("");
    } catch(e){ toast.error("No se pudo enviar la valoración"); }
    finally { setRatingSending(false); }
  }

  function clearFilters() {
    setFilterClubQuery(""); setFilterClubId(""); setFilterTeacher(""); setFilterPrice("all");
    const next=new URLSearchParams(searchParams); next.delete("teacher"); next.delete("club"); next.delete("clubName");
  ...
  const headerClubName = useMemo(() => {
    const n=String(searchParams.get("clubName")||"").trim(); if(n)return n;
  ...
  const teacherFilterOptions = useMemo(() => {
    ...
    return Array.from(set).map(id=>{ const p=profilesById?.[id]; return { id, name:(p?.name&&String(p.name).trim())||(p?.handle&&String(p.handle).trim())||`Profe ${id.slice(0,6)}…` }; }).sort((a,b)=>normText(a.name).localeCompare(normText(b.name)));
  }, [items,profilesById]);
      ...
      <div className="pageWrap">
        <div className="container" style={{padding:"0 16px",maxWidth:720,margin:"0 auto"}}>
          ...
          <div className="dayPicker">
            {nextDays.map(d=>(
              <button key={d.value} className={`dayBtn${day===d.value?" active":""}`} onClick={()=>setDay(d.value)}>
                <span style={{fontSize:10,fontWeight:800,textTransform:"uppercase",color:day===d.value?"#000":"rgba(255,255,255,0.5)"}}>{d.isToday?"hoy":d.label}</span>
                <span style={{fontSize:20,fontWeight:900,color:day===d.value?"#000":"#fff"}}>{d.num}</span>
              </button>
            ))}
          </div>
          ...
          <div style={{display:"flex",gap:6,marginBottom:10,flexWrap:"wrap"}}>
            <button className={`clBtn ${filtersOpen?"clBtnPrimary":"clBtnGhost"}`} style={{fontSize:11,padding:"6px 12px"}} onClick={()=>setFiltersOpen(f=>!f)}>
              🔍 Filtros{hasFilters?` (${[filterClubId,filterTeacher,filterPrice!=="all"].filter(Boolean).length})`:""}
            </button>
            {hasFilters && (
          ...
          {filtersOpen && (
            <div className="clSection" style={{marginBottom:10}}>
          ...
          {isTeacher && showCreateForm && (
            <div className="clSection">
              ...
              <div className="clGrid2">
                ...
                <div><label style={LB}>Precio (€)</label><input style={IS} type="number" value={slotPrice} onChange={e=>setSlotPrice(e.target.value)} placeholder="Ej: 30"/></div>
                <div><label style={LB}>Plazas</label><input style={IS} type="number" value={slotMaxStudents} min={1} max={10} onChange={e=>setSlotMaxStudents(e.target.value)} placeholder="1"/></div>
                <div><label style={LB}>Pista</label><input style={IS} value={slotCourt} onChange={e=>setSlotCourt(e.target.value)} placeholder="Ej: Pista 3"/></div>
                <div style={{gridColumn:"1 / -1"}}>
                  <label style={LB}>Club *</label>
                ...
                <div><label style={LB}>Ciudad / barrio</label><input style={IS} value={slotLocation} onChange={e=>setSlotLocation(e.target.value)} placeholder="Ej: Teatinos"/></div>
                <div><label style={LB}>Notas</label><input style={IS} value={slotNotes} onChange={e=>setSlotNotes(e.target.value)} placeholder="Ej: técnica + bandeja"/></div>
              </div>
              <button className="clBtn clBtnPrimary" style={{marginTop:14,width:"100%"}} onClick={createClass} disabled={!slotClubPick?.id}>Crear clase</button>
            </div>
          )}
          ...
          {loading ? (
            <div style={{textAlign:"center",padding:40,color:"rgba(255,255,255,0.4)"}}>
              <div style={{fontSize:32,marginBottom:8}}>⏳</div>Cargando clases…
            </div>
            ...
            <div style={{background:"#111",borderRadius:14,padding:28,textAlign:"center",border:"1px solid rgba(255,255,255,0.07)"}}>
              ...
              <div style={{color:"rgba(255,255,255,0.5)",fontSize:12,marginTop:4}}>Prueba otro día o cambia los filtros.</div>
            </div>
          ) : (
            <ul style={{listStyle:"none",padding:0,margin:0}}>
              {filteredItems.map(c=>{
                ...
                const isMyBooking=myBookings.has(String(c.id));
                ...
                const durationMins=c.end_at&&c.start_at?Math.round((new Date(c.end_at)-new Date(c.start_at))/60000):null;
                ...
                return (
                  <li key={c.id} className="clCard">
                    ...
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
                      <div>
                        <div style={{fontWeight:900,fontSize:22,color:"#fff"}}>{start} – {end}</div>
                    ...
                    <div onClick={()=>navigate(`/profesores/${c.teacher_id}`)} style={{display:"flex",alignItems:"center",gap:12,cursor:"pointer",marginBottom:12,padding:"10px 12px",borderRadius:12,background:"rgba(255,255,255,0.03)",border:"1px solid rgba(255,255,255,0.06)"}}>
                      ...
                      <div style={{flex:1}}>
                        ...
                        <div style={{fontSize:11,color:"rgba(255,255,255,0.4)"}}>Ver perfil →</div>
                      </div>
                      {favTeacherIds.has(String(c.teacher_id))&&<span style={{fontSize:18}}>⭐</span>}
                    </div>
                    ...
                    <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
                      {!booked ? (
                        ...
                        <>
                          <button className="clBtn clBtnMine" disabled style={{flex:1}}>✅ Reservada por ti</button>
                          <button className="clBtn clBtnGhost" onClick={()=>setRatingModal({classId:c.id,teacherId:c.teacher_id,teacherName:profName})} style={{fontSize:12}}>⭐ Valorar</button>
                          <button className="clBtn clBtnDanger" onClick={()=>cancelMyBooking(c.id)} disabled={bookingId===c.id} style={{fontSize:12}}>Anular</button>
                        </>
                      ) : (
        ...
        <div onClick={()=>setShowTeacherCode(false)} style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.88)",zIndex:40000,display:"flex",alignItems:"flex-end",justifyContent:"center",backdropFilter:"blur(4px)"}}>
          <div onClick={e=>e.stopPropagation()} style={{width:"100%",maxWidth:480,background:"#1a1a1a",borderRadius:"20px 20px 0 0",padding:"24px 20px 32px",border:"1px solid rgba(116,184,0,0.25)"}}>
            ...
            <div style={{display:"flex",gap:8,marginTop:16}}>
              <button className="clBtn clBtnPrimary" style={{flex:1}} onClick={confirmTeacherCode} disabled={teacherBusy}>{teacherBusy?"Activando…":"Activar"}</button>
        ...
        <div onClick={()=>setRatingModal(null)} style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.88)",zIndex:40000,display:"flex",alignItems:"flex-end",justifyContent:"center",backdropFilter:"blur(4px)"}}>
          <div onClick={e=>e.stopPropagation()} style={{width:"100%",maxWidth:480,background:"#1a1a1a",borderRadius:"20px 20px 0 0",padding:"24px 20px 32px",border:"1px solid rgba(116,184,0,0.25)"}}>
            ...
            <div style={{display:"flex",justifyContent:"center",gap:4,marginBottom:20}}>
              {[1,2,3,4,5].map(star=>(
                <button key={star} className="starBtn" onClick={()=>setRatingValue(star)}
                  style={{opacity:ratingValue>=star?1:0.25,transform:ratingValue>=star?"scale(1.15)":"scale(1)"}}>⭐</button>
              ))}
            </div>
            {ratingValue>0&&(
              <div style={{marginBottom:16}}>
                <div style={{fontSize:11,fontWeight:800,color:"rgba(255,255,255,0.4)",marginBottom:8}}>COMENTARIO (opcional)</div>
                <textarea value={ratingComment} onChange={e=>setRatingComment(e.target.value)} rows={2} placeholder="¿Qué destacarías de la clase?"
                  style={{width:"100%",padding:"10px 12px",borderRadius:10,background:"rgba(255,255,255,0.06)",border:"1px solid rgba(255,255,255,0.12)",color:"#fff",fontSize:13,resize:"none",outline:"none",boxSizing:"border-box"}}/>
              </div>
            )}
            <div style={{display:"flex",gap:8}}>
              {ratingValue>0&&(
                <button className="clBtn clBtnPrimary" style={{flex:1}} onClick={sendRating} disabled={ratingSending}>
                  {ratingSending?"Enviando…":"✅ Enviar valoración"}
                </button>
              )}
              <button className="clBtn clBtnGhost" onClick={()=>setRatingModal(null)}>Cancelar</button>
            </div>
          </div>
        </div>
      )}
